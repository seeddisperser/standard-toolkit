...
import { type Format, BEARINGS, LIMITS, Symbols } from '../internal';
import { addTo } from '../internal/add-to';
import type { ParseResults } from '../internal/parse';

import type { DegreesDecimalMinutes } from './degrees-decimal-minutes';
...
export function identifyErrors(format: Format) {
  ...
  return (arg: DegreesDecimalMinutes | undefined, i: number) => {
    ...
    addTo(
      errors,
      59.999999999 < Number.parseFloat(min),
      `Minutes value "${min}" exceeds max value "59.999999999".`,
    );
    ...
    addTo(
      errors,
      [deg, min, bear].join(' ').includes(Symbols.Seconds),
      `Seconds indicator (${Symbols.Seconds}) not valid in Degree Decimal Minutes.`,
    );
