...
describe('Patterning', () => {
  ...
  describe('limn', () => {
    it('should build a regex from parts', () => {
      ...
      expect(Patterning.limn(parts, 'beta alpha').toString()).toBe(
        /^beta alpha$/.toString(),
      );

      expect(Patterning.limn(parts, 'alpha beta missing').toString()).toBe(
        /^alpha beta ___UNMATCHED_missing_TOKEN___$/.toString(),
      );
    });
  });
});
