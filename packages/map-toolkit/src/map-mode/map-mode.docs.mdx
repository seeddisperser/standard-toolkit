...
**React Integration:**
- `react` - React 19.0.0 or higher for the `useMapMode` hook and `MapProvider`
...
The `BaseMap` component automatically creates a `MapProvider` internally and provides map mode functionality. Create a unique `id` for the map instance as a **module-level constant** (recommended):
...
**Why module-level?** Creating `id` as a constant outside your components ensures it's stable and makes it easy to share across all components that need to access the map mode.

The map mode always starts in `'default'` mode and is ready to use immediately.

### 2. Use the mode in your UI components

Access the current mode and request mode changes using the `useMapMode` hook. **Important**: UI components (buttons, toolbars, etc.) should be rendered as **siblings** to `BaseMap`, not children, and must pass the `id` to `useMapMode()`:
...
function DrawingToolbar() {
  // Use the shared id to access the map mode state
...
Any component can read the current mode and react to changes. All UI components should be siblings to `BaseMap` and use the same shared `id` constant:
...
When you need multiple independent maps on the same page (e.g., main map + minimap), provide unique `id` props to each `BaseMap`. UI components must pass the corresponding `id` to `useMapMode()`:
...
- Each `BaseMap` with a unique `id` (UUID) operates independently
- Mode changes in one instance don't affect other instances
- UI components must pass the correct `id` to `useMapMode()` to control the desired map
- Events are automatically scoped to each instance via `id` in event payloads
- The `id` is required and must be provided by the consumer to enable map mode coordination
...
When your features need to coordinate mode changes, implement authorization handlers using the shared module-level `id`:
...
function DrawingFeature() {
  // Use shared id to access mode state
  const { mode, requestModeChange } = useMapMode(MAIN_MAP_ID);
  ...
  useOn<ModeChangeAuthorizationEvent>(
    ...
    (event) => {
      ...
      if (event.payload.id !== MAIN_MAP_ID) return;
...
Hook to access map mode state. **For UI components (buttons, toolbars, etc.)**, you should pass the `id` parameter since they must be rendered as siblings to `BaseMap`, not children.

#### Parameters

##### `id?: UniqueId`

The map ID to access. **Recommended**: Always pass this parameter for UI components that are siblings to `BaseMap`. If not provided, the hook will attempt to use the ID from `MapContext` (only available for Deck.gl layer components that are children of `BaseMap`).
...
- `Error` if no `id` is provided and hook is used outside of `BaseMap`
...
```tsx
{
  ...
  id: UniqueId;   // The map instance this event is for
}
```
...
```tsx
{
  ...
  id: UniqueId;  // The map instance this event is for (REQUIRED)
}
```

**Important:** Always include the `id` from the authorization event in your decision to ensure it's scoped to the correct map instance.
...
### Error: "useMapMode requires either an id parameter or to be used within a MapProvider"

**Cause**: Using `useMapMode()` without providing an `id` parameter, outside of a `MapProvider` context.

**Solution**: Pass the `id` to the hook. UI components should be siblings to `BaseMap` and use the shared module-level constant:
...
**Note**: Only custom Deck.gl layer components rendered as children of `BaseMap` can use `useMapMode()` without parameters (via context). Regular UI components must pass `id`.

### Error: "MapModeStore not found for map instance: [id]"

**Cause**: Calling `useMapMode(id)` with an ID that doesn't have a store created yet, or after the store was destroyed.

**Solution**: Ensure `BaseMap` with the matching `id` is mounted before any components try to use `useMapMode` with that ID.

### ID Changes Cause Remount

**Behavior**: When the `id` prop passed to `BaseMap` changes, the component performs a complete remount.

**Impact**:
- The old store is properly cleaned up (no memory leaks)
- All child components remount with the new ID
- Map state is reset to default
- All pending mode change requests are cleared

**Best Practice**: While ID changes are handled safely, `id` should typically remain stable using a module-level constant:

```tsx
// ✅ Recommended - stable ID for the lifetime of the app
const MAIN_MAP_ID = uuid();

export function App() {
  return <BaseMap id={MAIN_MAP_ID} />;
}

// ⚠️ Works but triggers remount - only use if you need dynamic IDs
function App() {
  const [mapId] = useState(() => uuid());
  return <BaseMap id={mapId} />;
}

// ❌ Bad - recreates on every render, constant remounting
function App() {
...
**Cause**: Not using unique `id` props for each `BaseMap`.
...
<BaseMap id={MAP_1_ID}>...</BaseMap>
<BaseMap id={MAP_2_ID}>...</BaseMap>
```
