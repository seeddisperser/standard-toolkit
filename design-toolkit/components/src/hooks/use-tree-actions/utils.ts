 ...
 */
export function validateCache<T extends object>(
  nodes: TreeNode<T>[],
 ...
 */
export function toTree<T extends object>(): TreeNode<T>[] {
 ...
 * @param parentKey
 */
export function buildLookup<T extends object>(
  nodes: TreeNode<T>[],
  lookup: TreeMap<T>,
  parentKey?: Key | null,
) {
 ...
 * outside this file.
 **/
export function getNode<T>(key: Key): TreeNode<T> {
...
export function setNode<T>(key: Key, node: TreeNode<T>) {
  _setNode(key, {
    ...node,
    children: (node.children ?? []).map((child) => child.key),
  });
...
export function setAllNodes<T>(patch: Partial<TreeNode<T>>) {
...
export function deleteNode(key: Key) {
  const node = cache.lookup.get(key);
...
export function move(parentKey: Key | null, key: Key, idx: number) {
  const node = _getNode(key);
...
export function setViewable(key: Key, state: boolean) {
  const node = cache.lookup.get(key);
  if (node) {
    _setNode(key, {
      ...node,
