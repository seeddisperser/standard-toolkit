...
* **`children`** - Must contain an `AccordionTrigger` and optionally other elements like menus
...
The `Accordion` component wraps the `Disclosure` component from `react-aria-components`.
...
[Github Source](https://github.com/gohypergiant/standard-toolkit/tree/main/design-toolkit/components/src/components/accordion)
