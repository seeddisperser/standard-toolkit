...
- **Tree.Item.Content** - composes the actual display of the tree node content. `Tree.Item` will always have `Tree.Item.Content` composed inside it.
- **Tree.Item.Label** - places and styles the tree node label
- **Tree.Item.Description** - places and styles the tree description
- **Tree.Item.PrefixIcon** - places and styles the icon that optionally shows before a tree node
- **Tree.Item.Actions** - container for unique application icons for a tree item
...
const Node = ({ node }) => (
  <Tree.Item id={node.key} textValue={node.label}>
    <Tree.Item.Content>
...
The Tree component can be used in either controlled or uncontrolled mode. In controlled mode, you manage the state externally and pass it to the component via props. In uncontrolled mode, the component manages its own state internally.

### Controlled State

To use the Tree component in controlled mode, provide the appropriate state props and callbacks:
...
When working with dynamic data, you can use the `items` prop and a render function as children to efficiently render large trees:
...
const renderNode = (node) => (
  <Tree.Item id={node.key} textValue={node.label}>
    <Tree.Item.Content>
      <Tree.Item.Label>{node.label}</Tree.Item.Label>
      {node.description && (
        <Tree.Item.Description>{node.description}</Tree.Item.Description>
      )}
    </Tree.Item.Content>
...
1. **Static Collection**: Hard-coded tree structure using nested Tree.Item components
...
Instead, when using dynamic data, let the component manage its state internally or use the state management hooks from the `use-tree` package.

### useTreeActions

The `useTreeActions` hook provides a set of utility functions for manipulating tree data. It's particularly useful when working with complex tree structures that require advanced operations like inserting, removing, or moving nodes.

```jsx
import { useTreeActions } from '@/hooks/use-tree';

function MyTreeComponent() {
  const [treeData, setTreeData] = useState(initialTreeData);
  const { actions } = useTreeActions({ nodes: treeData });

  const handleInsertNode = (targetKey, newNode) => {
    const updatedTree = actions.insertInto(targetKey, [newNode]);
    setTreeData(updatedTree);
  };

  const handleRemoveNode = (key) => {
    const updatedTree = actions.remove(new Set([key]));
    setTreeData(updatedTree);
  };
...
The `useTreeState` hook provides a complete state management solution for tree data. It manages expansion, selection, and visibility states, and provides actions for manipulating the tree.
...
- **Page Up/Down**: Move focus up/down by a page
- *** (asterisk)**: Expand all siblings of the current node
...
The `Tree` is build on the React Aria Tree headless component. [Docs](https://react-spectrum.adobe.com/react-aria/Tree.html)
