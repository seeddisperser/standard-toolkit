...
- **TextFieldProvider** - Context provider for setting default text field props
...
The size can be controlled via the `size` prop or set globally using the `TextFieldProvider` component.
...
The `TextField` component wraps the `TextField` component from `react-aria-components` and integrates with the `Input` component.

[Docs](https://react-spectrum.adobe.com/react-aria/TextField.html)

[Github Source](https://github.com/gohypergiant/standard-toolkit/tree/main/design-toolkit/components/src/components/text-field)
