...
- **LinkProvider** - Context provider for setting default props across multiple links
...
<LinkProvider allowsVisited className="enabled:visited:fg-accent-primary-pressed enabled:visited:underline">
...
### Context Provider
Use `LinkProvider` to set consistent props for multiple links:
```tsx
...
```tsx
<LinkProvider className="visited:fg-accent-primary-pressed visited:underline">
  <Link allowsVisited href="/styled-visited">
    Styled Visited Link
  </Link>
</LinkProvider>
...
- **Icon Integration**: Automatic icon sizing through `IconProvider` context
- **Context Provider**: `LinkProvider` for consistent props across multiple links
