...
- **Label** - Main label component with automatic optional/required text handling
- **LabelProvider** - Context provider for setting consistent styling across multiple labels
...
// Using LabelProvider for consistent styling
<LabelProvider isRequired={true}>
...
### Context Provider
Use `LabelProvider` to set consistent styling for multiple labels:
```tsx
<LabelProvider isRequired={true}>
...
[Docs](https://react-spectrum.adobe.com/react-aria/TextField.html#label)

[Github Source](https://github.com/gohypergiant/standard-toolkit/tree/main/design-toolkit/components/src/components/label)
...
- **Context Provider**: LabelProvider for consistent styling across multiple labels
