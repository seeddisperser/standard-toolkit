...
## Props
The NoticeList component accepts all standard HTML div attributes
...
## Events
The NoticeList implements events via bus
...
The `NoticeList` component wraps the `ToastRegion` and `ToastQueue` components from `react-aria-components` and integrates with a custom event bus system for queue management.
