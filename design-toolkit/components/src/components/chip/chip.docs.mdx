...
* **Basic Chip** - simple display chip for showing tags or labels
* **Selectable Chip** - interactive chip that can be selected/deselected
* **Deletable Chip** - chip with a remove button for dynamic content management
...
* **`selectionMode`** - Selection behavior: `'none'`, `'single'`, or `'multiple'`
...
### Chip.Provider
Context provider component that allows setting default props for multiple Chip components. Useful for maintaining consistent sizing and styling across a group of chips.

### Chip.List
Container component that groups multiple chips together and provides shared configuration like size, selection behavior, and remove functionality.

### Chip.Selectable
Interactive chip that can be selected and deselected, perfect for filtering interfaces and multi-selection scenarios.

### Chip.Deletable
Chip with a built-in remove button, ideal for dynamic content management and user-generated tags.

## Implementation Notes
The `Chip` component wraps the `Tag` component from `react-aria-components`. [Docs](https://react-spectrum.adobe.com/react-spectrum/Tag.html)
