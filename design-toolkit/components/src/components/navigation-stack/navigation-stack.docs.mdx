...
- `[any combination of the above]`: Each value will rules as stated above. Each event will be fired in sequence in the order provided.

## Implementation Notes
To gaurantee uniqueness of ids across an application, the NavigationStack implements Tagged UUID strings. This is also validated and will throw an error if a non-UUID is passed in.
